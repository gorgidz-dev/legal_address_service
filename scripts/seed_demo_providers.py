...
async def _seed(db, *, count: int, photo_dir: pathlib.Path, admin: User) -> dict[str, int]:
    ...
    for index, (city, street, room, fns_number, fns_city) in enumerate(listings):
        ...
        provider = Provider(
            ...
            full_name=full_name,
            # ИНН/ОГРН намеренно пустые.
            #
            # Первая версия сидера выдумывала их «похожими на настоящие», и
            # схема ProviderRead, проверяющая контрольные суммы, роняла
            # GET /providers в 500 — админский раздел «Собственники» переставал
            # открываться целиком. Придумать валидные тоже нельзя: такой номер
            # может совпасть с реально существующей организацией.
            #
            # У витринной карточки регистрационных данных просто нет — они
            # появятся вместе с реальным собственником.
            inn=None,
            kpp=None,
            ogrn=None,
