...
class Settings(BaseSettings):
    ...
    # Куда банк вернёт покупателя после оплаты. Пусто → страница сайта
    # /payment/{success|fail}?application=<id> (ведёт в карточку заявки).
    ...
    # Чеки 54-ФЗ через облачную кассу, привязанную к терминалу в ЛК Т-Бизнеса
    # (выбрана OFD Ferma). Два чека: «предоплата 100%» при оплате (Receipt в
    # Init) и «полный расчёт» при выдаче документов (/cashbox/SendClosingReceipt).
    # Включать ТОЛЬКО вместе с подключением кассы в ЛК: при подключённой кассе
    # Receipt в Init обязателен, без кассы чек некому пробить. Разбор — §7 и §10
    # docs/tbank-acquiring.md.
    tbank_receipts_enabled: bool = False
    tbank_cashbox_url: str = "https://securepay.tinkoff.ru/cashbox"
    # Тег 1055. ООО на общей системе налогообложения.
    tbank_receipt_taxation: Literal[
        "osn", "usn_income", "usn_income_outcome", "esn", "patent"
    ] = "osn"
    # Ставка НДС полного расчёта. В чеке предоплаты — расчётная (22/122 и т.п.),
    # она выводится из этой автоматически (письмо ФНС № АБ-4-20/11248@).
    tbank_receipt_vat: Literal["none", "vat0", "vat5", "vat7", "vat10", "vat22"] = "vat22"
    # Тег 2108 (обязателен в ФФД 1.2). «шт» — по описанию спецификации для
    # предметов расчёта, реализуемых единицами; вопрос к бухгалтеру — §10 дока.
    tbank_receipt_measurement_unit: str = "шт"
    # Тег 1030, до 128 символов. Формулировка — предмет договора из оферты.
    tbank_receipt_item_name: str = "Услуги по подбору юридического адреса и подготовке документов"
    # Версия ФФД в Receipt. В OpenAPI v1.32 поля нет (версию берут из ЛК), но
    # прежняя документация требовала FfdVersion=1.2 для касс на ФФД 1.2 и
    # считала умолчанием 1.05. «1.2» корректно в обоих прочтениях, если касса
    # и ЛК на 1.2 (так и настраиваем). Пусто — не передавать.
    tbank_receipt_ffd_version: str = "1.2"
    # Сколько раз фоновая сверка повторяет закрывающий чек после явного отказа
    # банка, прежде чем оставить это админу.
    tbank_closing_receipt_max_attempts: int = 5
